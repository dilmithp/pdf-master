...
import com.pdfmaster.pdfai.audit.SpringDataAuditLogRepository;
...
/**
 * Verifies that every HTTP request causes an audit_log row to be persisted within 1s.
 */
@SpringBootTest
...
class AuditFilterIntegrationTest {
  ...
  static final String BUCKET = "pdfmaster-audit-test";
  ...
  void auditRowIsPersistedAfterRequest() throws Exception {
    ...
    mockMvc
        .perform(get("/v1/jobs/" + UUID.randomUUID()).with(jwt()))
        .andReturn();
