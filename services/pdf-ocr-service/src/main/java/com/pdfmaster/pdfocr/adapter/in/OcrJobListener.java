...
/**
 * Consumes {@code pdf.ocr.requested}. Validates PDF magic-bytes, runs OCR via the configured {@link
 * OcrEngine} (Noop in tests, Tesseract in production), uploads the extracted text.
 */
@Component
public class OcrJobListener {
  ...
  public void onMessage(Map<String, Object> message) {
    ...
    try {
      ...
      String outputKey = "results/" + jobId.value() + "/text.txt";
      objectStore.upload(
          outputKey,
          textBytes,
      ...
      LOG.info("OCR job {} succeeded ({} chars)", jobId, text.length());
